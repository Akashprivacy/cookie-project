...
export const URLInputForm: React.FC<URLInputFormProps> = ({ url, setUrl, scanDepth, setScanDepth, onScan, isLoading }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full bg-[var(--bg-secondary)] p-6 rounded-xl border border-[var(--border-primary)] shadow-sm">
      <div className="space-y-4">
        ...
        {scanDepth !== undefined && setScanDepth && (
          <div>
            <label htmlFor="scan-depth-slider" className="block text-sm font-medium text-[var(--text-primary)] mb-2">
              Scan Depth: <span className="font-bold text-[var(--text-headings)]">{scanDepth} page{scanDepth > 1 ? 's' : ''}</span>
            </label>
            <input
              ...
              disabled={isLoading}
              className="w-full h-2 bg-[var(--bg-tertiary)] rounded-lg appearance-none cursor-pointer accent-brand-blue disabled:opacity-50"
            />
          </div>
        )}
      </div>
